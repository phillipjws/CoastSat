...
def initial_settings(sitename, s2tile):
    """
...
def main():
    for filename in os.listdir(r'D:\Inputs'):
        if filename.startswith('MASSET_3') and filename.endswith('.kml'):
            sitename = filename[:-4]
            print(f'Starting site: {sitename}')
            if sitename == 'MASSET_3' or sitename == 'MASSET_4':
                s2tile = '09UUA'
            else:
                s2tile = '09UUV'

            inputs, settings, metadata = initial_settings(sitename, s2tile)
