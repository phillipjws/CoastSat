...
def initial_settings():
    ...
    geojson_polygon = os.path.join(os.pardir, 'planetscope_coastsat', 'user_inputs','upper_james_island.kml')
...
def batch_shoreline_detection(metadata, settings, inputs):
    ...
    settings['reference_shoreline'] = SDS_preprocess.get_reference_sl_from_geojson(settings['inputs']['sitename'], os.path.join(r'U:\ref_shorelines\singlepart'), settings['output_epsg'])
...
def tidal_correction(output, cross_distance, transects, settings):
    ...
    return cross_distance_tidally_corrected, dates_sat, dates_ts, tides_sat, tides_ts
...
def slope_estimation(settings, cross_distance, dates_sat, dates_ts, tides_sat, tides_ts):
...
def main():
    ...
    cross_distance_tidally_corrected, dates_sat, dates_ts, tides_sat, tides_ts = tidal_correction(output, cross_distance, transects, settings)
    cross_distance = time_series_post_processing(transects, settings)
    slope_estimation(settings, cross_distance, dates_sat, dates_ts, tides_sat, tides_ts)
