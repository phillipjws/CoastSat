...
def initial_settings():
    ...
    geojson_polygon = os.path.join(os.getcwd(), 'sidney_island.geojson')
...
def estimate_slope(settings, transects, filepath):
    ...
    filename_output = os.path.join(os.getcwd(), 'data', settings['inputs']['sitename'], f'{settings['inputs']['sitename']}_output.pkl')
    with open(filename_output, 'rb') as f:
        output = pickle.load(f)

    output = SDS_tools.remove_duplicates(output)

    output = SDS_tools.remove_inaccurate_georef(output)
    ...
    settings_outliers = {'max_cross_change': 40,
