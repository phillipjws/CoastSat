...
def initial_settings(sitename):
    ...
    settings = {
        ...
        's2cloudless_prob': 90,  # Threshold to identify cloud pixels in the s2cloudless probability mask
...
def batch_shoreline_detection(metadata, settings, inputs):
    ...
    settings['reference_shoreline'] = SDS_preprocess.get_reference_sl_from_geojson(f"REFERENCE_SHORELINE_{settings['inputs']['sitename']}", os.path.join(r'D:\Inputs\3157\reference_shorelines'), settings['output_epsg'])
    # Set the max distance (in meters) allowed from the reference shoreline for a detected shoreline to be valid
    settings['max_dist_ref'] = 350
...
def calculate_and_save_trends(transects, cross_distance_tidally_corrected, output, settings, slope_est, trend_dict):
    ...
    for key, geometry in transects.items():
        ...
        slope_energy_curve_path = os.path.join(settings['inputs']['filepath'], settings['inputs']['sitename'], 'slope_estimation', slope_energy_curve_filename)
...
def main():
    for filename in sorted(os.listdir(r'D:\Inputs\3157')):
        if filename.startswith(('BC_0', 'BC_1', 'BC_2', 'BC_3', 'BC_4', 'BC_5', 'BC_9'), 0, 4) and filename.endswith(('BC_21.kml', 'BC_22.kml', 'BC_23.kml')):
            sitename = filename[:-4]
