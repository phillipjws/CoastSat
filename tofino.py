...
def shoreline_analysis(output, settings):
    ...
    geojson_transects = os.path.join(
        r'D:\Inputs\3157\transects', f"TRANSECTS_{settings['inputs']['sitename']}_LONG.geojson"
    )
...
def main():
    for filename in sorted(os.listdir(r'D:\Inputs\3157')):
        if filename.startswith(('BC_0', 'BC_1', 'BC_2', 'BC_3', 'BC_4', 'BC_5', 'BC_9'), 0, 4) and filename.endswith('BC_32.kml'):
            sitename = filename[:-4]
